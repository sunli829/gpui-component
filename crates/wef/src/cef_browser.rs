...
use crate::{BrowserBuilder, Frame, KeyCode, KeyModifier, MouseButton, ffi::*};
    ...
    pub fn send_mouse_click_event(
        ...
        unsafe {
            wef_browser_send_mouse_click_event(
                ...
                mouse_up,
                (click_count as i32).min(2),
            )
        };
    }

    /// Sends a mouse move event.
    pub fn send_mouse_move_event(&self, x: i32, y: i32) {
        unsafe { wef_browser_send_mouse_move_event(self.wef_browser, x, y) };
    }

    /// Sends a mouse wheel event.
    pub fn send_mouse_wheel_event(&self, delta_x: i32, delta_y: i32) {
        unsafe { wef_browser_send_mouse_wheel_event(self.wef_browser, delta_x, delta_y) };
    }
