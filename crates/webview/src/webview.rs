...
use wef::{Browser, FuncRegistry, Rect};
...
thread_local! {
    static WEF_EVENT_LOOP_INIT: Cell<bool> = const { Cell::new(false) };
}
const WEF_WORK_INTERVAL: Duration = Duration::from_millis(14);
...
impl WebView {
            ...
            cx.foreground_executor()
                .spawn(async move {
                    loop {
                        ...
                        async_cx
                            .background_executor()
                            .timer(WEF_WORK_INTERVAL)
                            .await;
                    }
                })
                .detach();
            WEF_EVENT_LOOP_INIT.set(true);
        }
    ...
    fn scroll_wheel_handler(
        ...
        self.browser()
            .send_mouse_wheel_event((delta_x) as i32, (delta_y) as i32);
    }
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if let Some(info) = &self.context_menu {
            root = root.child(deferred(
                anchored()
                    .position(point(px(info.crood.x as f32), px(info.crood.y as f32)))
                    .child(
                        div()
