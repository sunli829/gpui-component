...
use wef::{
    ...
    JsDialogCallback, JsDialogType, PaintElementType, Point, Rect,
};
            ...
            _ = self.cx.update_window(self.window_handle, |_, window, cx| {
                cx.update_entity(&entity, |webview, cx| {
                    webview.context_menu = Some(ContextMenuInfo {
                        crood: Point {
                            x: params.crood.x + webview.bounds.origin.x.0 as i32,
                            y: params.crood.y + webview.bounds.origin.y.0 as i32,
                        },
                        frame,
